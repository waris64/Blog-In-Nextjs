...
const BlogData = () => {
    ...
    const del = async (id) => {
        try {
            ...
            if (consent) {
                ...
                console.log("Consent is true . Speaking from DEL function in index.jsx")
                const hit = await fetch(`/api/blogs/${id}`, { method: 'DELETE' });
                console.log("Sending DELETE request for ID:", id);
                if (!hit.ok) {
                    const errorDetails = await hit.text();
                    console.log(`Delete failed with status : ${hit.status} - ${hit.statusText}`)
                    throw new Error(errorDetails || "Failed to delete the blog ")
                }

                console.log(`Blog with ID: ${id} deleted`);
                toast.success("Blog Deleted Successfully");
                setBlogsData(prevBlogData => prevBlogData.filter(blog => blog._id !== id));
                // else {
    ...
    return (
        <div>
            <h1 className="text-center text-3xl font-bold mt-5 mb-5">Welcome to Admin Panel</h1>
            <Toaster />
                                ...
                                <table className="min-w-full rounded-xl">
                                    <thead>
                                        <tr className="bg-gray-50">
                                            {/* <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl">ID</th> */}
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Title</th>
                                            <th className="px-4 py-2 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">
                                                Image
                                            </th>
                                    ...
                                    <tbody className="divide-y divide-gray-300">
                                        {blogsData.map((blog) => (
                                            <tr key={blog._id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                                                {/* <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{blog._id}</td> */}
                                                <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"><Link href={`/blogs/${blog._id}`}> {blog.title}</Link></td>
                                                <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div className="w-full h-[200px]  overflow-hidden  bg-red-400 mx-auto mb-4">
                                                        <Image src={`/uploads/${blog.file}`} width={0} height={0} alt='Blog Image ' className="object-cover w-full h-full transform transition-transform ease-in-out duration-400 hover:scale-110" />
                                                    </div>
                                                </td>
                                                <td className="p-5">
